...
export default async function handler(req: any, res: any) {
  ...
  try {
    ...
    const insertRes = await fetch(`${SUPABASE_URL}/rest/v1/user_tasks`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json', Prefer: 'return=representation', ...sbHeaders },
      body: JSON.stringify({ user_id, task_id, started_at: now, status: 'verifying' }),
    });
    ...
    if (!taskRecord) return res.status(502).json({ error: 'No record returned from insert' });
